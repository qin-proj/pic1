...
vcell=pbmc@assays$RNA@data[,pbmc$subclass==cl & pbmc$species=="Rhesus"]
...
write_csv(frame,file = paste0("Rhesus/", cl, ".csv"))
vcell=pbmc@assays$RNA@data[,pbmc$subclass==cl & pbmc$species=="Human"]
exp=vcell[,1:ncol(vcell)]
dimnames=list(rownames(exp),colnames(exp))
data=matrix(as.numeric(as.matrix(exp)),nrow=nrow(exp),dimnames=dimnames)
data2=t(data)
...
write_csv(frame,file = paste0("Human/", cl, ".csv"))
}
